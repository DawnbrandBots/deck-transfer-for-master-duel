...
test("Streamlined import functional test 3", async ({ ygoprodeckExport }) => {
  await expect(ygoprodeckExport.url().startsWith("https://www.db.yugioh-card.com/yugiohdb/member_deck.action#storm-access=")).toBe(true);
  await expect(ygoprodeckExport.locator("dialog")).toContainText("You are not logged into Konami's official database. Please log in first and then try exporting again.", { timeout: 10000 });
});
